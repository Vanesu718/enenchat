    ...
    bubble.innerHTML = `
      <div class="blue-card-top">
        ...
        <div class="blue-card-status">
          ...
          <div class="bc-status-item favor"><div class="bc-label">?? 好感度</div><div class="bc-val">${statusData?.favor || 0}%</div></div>
        </div>
      </div>
  ...
  visibleMoments.forEach((moment, mIdx) => {
    ...
    div.innerHTML = `
      ...
      <div class="moment-actions">
        <div class="moment-action-btn" onclick="likeMoment('${moment.id}')">${(moment.likes && moment.likes.indexOf('我') > -1) ? '<span style="color:var(--main-pink);">?? 已赞</span>' : '?? 赞'}</div>
        <div class="moment-action-btn" onclick="commentMoment('${moment.id}')">?? 评论</div>
        <div class="moment-action-btn delete-moment" onclick="deleteMoment('${moment.id}')">??? 删除</div>
      </div>
      ${commentsHtml}
    `;
...
function updateMomentLikes(momentId) {
  ...
  if (likeBtn && moment.likes) {
    ...
    likeBtn.innerHTML = liked ? '<span style="color:var(--main-pink);">?? 已赞</span>' : '?? 赞';
  }
}
  ...
  stm.entries.forEach((entry, idx) => {
    ...
    div.innerHTML = `
      ...
      <div style="display:flex; justify-content:space-between; align-items:center;">
        ...
        <div onclick="event.stopPropagation(); openEditStm(${idx})" style="font-size:12px; color:var(--main-pink); padding:4px 12px; background:var(--light-pink); border-radius:8px; display:${isStmBatchDeleteMode?'none':'block'};">?? 编辑</div>
      </div>
    `;
