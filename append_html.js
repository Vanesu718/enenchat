...
// Insert the new windows before the chat-settings-page or personal-setting
html = html.replace(/<div class="sub-page" id="userSetting">/, writingStyleWin + '\n    <div class="sub-page" id="userSetting">');

const chatSettingsInject = `
        <!-- 文风选择 -->
...
html = html.replace(/<!-- 用户人设设定 -->/, chatSettingsInject + '\n        <!-- 用户人设设定 -->');

const groupChatSettingsInject = `
        <!-- 文风选择 -->
        <div class="setting-section">
          <div class="setting-section-title">文风选择</div>
...
html = html.replace(/<!-- 用户人设设定 -->/g, function(match, offset, string) {
    // Only replace the second occurrence for group chat
    if (offset > html.indexOf('id="group-chat-settings-page"')) {
        return groupChatSettingsInject + '\n        <!-- 用户人设设定 -->';
    }
    return match;
});
