...
async function generatePersonaMemo(memoId, personaId) {
  ...
  const prompt = `请将以下长篇人设浓缩成 1-2 句话（最多100字），用于作为核心设定与当前环境备忘录，随时提醒大模型保持核心人设和状态：\n\n${personaText}\n\n只返回纯文本摘要，不要加任何解释或引号。`;
...
async function saveContact() {
  ...
  contacts.push({ id: Date.now().toString(), name, group, persona: p, memo, avatar });
  try {
    await saveToStorage('CHAT_CONTACTS', JSON.stringify(contacts));
    renderContactList();
    showToast('? 联系人已永久保存！');
...
function openChatSettings() {
  ...
  if (currentContact && !currentContact.isGroup) {
    const chatContactMemo = document.getElementById('chatContactMemo');
    if (chatContactMemo) {
      chatContactMemo.value = currentContact.memo || '';
    }
  }
...
async function saveAllChatSettings() {
  ...
  const newContactMemo = document.getElementById('chatContactMemo')?.value.trim();
