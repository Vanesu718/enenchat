...
$initCode = "      const rawWritingStyles = await getFromStorage('WRITING_STYLES');
      try {
...
if ($content.Contains('let writingStyles = [];') -and -not $content.Contains("const rawWritingStyles = await getFromStorage('WRITING_STYLES');")) {
...
if (-not $content.Contains('function renderWritingStyleList()')) {
    $content = $content.Replace("function renderWorldBookList() {", $wsFunctions + "`nfunction renderWorldBookList() {")
}

[System.IO.File]::WriteAllText("js\main.js", $content, (New-Object System.Text.UTF8Encoding($false)))
Write-Host "done"