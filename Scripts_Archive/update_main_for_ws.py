...
def update_main_js():
    ...
    if 'let writingStyles = [];' in content and 'const rawWritingStyles = await getFromStorage('WRITING_STYLES');' not in content:
        init_pos = content.find("const rawWbEntries = await getFromStorage('WORLDBOOK_ENTRIES');")
        if init_pos != -1:
            init_code = """
    ...
    if 'function renderWritingStyleList()' not in content:
        ws_functions = """
  // ================= 文风管理逻辑 =================
...
"""
        ...
        insert_target = "function renderWorldBookList() {"
        pos = content.find(insert_target)
        if pos != -1:
            content = content[:pos] + ws_functions + "
" + content[pos:]
