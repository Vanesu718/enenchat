...
// 1. Update chatSettings default
content = content.replace(`    chatSettings = {
      hideAvatar: false,
      chatBg: '',
      chatUserAvatar: '',
      chatNickname: '',
...
// 2. Append styleSelect to initChatSettingsPage
const target1 = `  // 联系人设定：优先使用已保存的设定，若为空则回退到创建联系人时输入的人设
  ...
  // 加载联系人头像和名字`;
const replace1 = `  // 联系人设定：优先使用已保存的设定，若为空则回退到创建联系人时输入的人设
  const currentContact = contacts.find(c => c.id === currentContactId);
  document.getElementById('contactMaskTextarea').value = chatSettings.contactMask || (currentContact ? currentContact.persona : '') || '';
...
// 3. Append styleSelect to initGroupChatSettingsPage
const target2 = `  // 场景设定和用户人设
...
// 4. Add handleWritingStyleChange
const target3 = `// 预览聊天背景文件
function previewChatBgFile(input) {`;
const replace3 = `// 处理文风切换
...
console.log('Main.js updated successfully via node.');