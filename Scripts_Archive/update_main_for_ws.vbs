function update_main_js() {
    var fso = new ActiveXObject("Scripting.FileSystemObject");
    var ForReading = 1;
    var ForWriting = 2;
    var file = fso.OpenTextFile("js\main.js", ForReading, false, -1);
    ...
    if (content.indexOf('let writingStyles = [];') !== -1 && content.indexOf("const rawWritingStyles = await getFromStorage('WRITING_STYLES');") === -1) {
        var initCode = "      const rawWritingStyles = await getFromStorage('WRITING_STYLES');
" +
"      try {
" +
...
"  }
";
        content = content.replace('function renderWorldBookList() {', wsFunctions + "
function renderWorldBookList() {");
    }

    var outFile = fso.OpenTextFile("js\main.js", ForWriting, false, -1);
